...
// This example will use the 2Input PnP module and will print its input changes to the serial monitor
...
// In this example we use the same callback function for both the inputs and determine which line caused the event
// by calling the GetEventIndex(). We could have different callbacks for each input line as well.
...
{
	serial.print("Input ");
	serial.print(inputModule.GetEventIndex());
	serial.print(" changed to: ");
	serial.println(inputModule.GetLastValue(inputModule.GetEventIndex()));
}
...
void setup()
{
	...
	for (uint8_t i=0; i<inputModule.numberOfInputs; i++) {
		inputModule.SetOnChange(i, onInputChange);
	}
