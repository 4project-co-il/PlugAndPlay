#include "PnP_Module_4Input.h"

PnP_Module_4Input::PnP_Module_4Input() : EBF_Module_4Input(NULL)
{
	this->type = HAL_Type::PnP_DEVICE;
	this->id = PnP_DeviceId::PNP_ID_4INPUT;
}

uint8_t PnP_Module_4Input::Init()
{
	uint8_t rc = EBF_OK;
...
void PnP_Module_4Input::ExecuteCallback()
{
...
uint8_t PnP_Module_4Input::Process()
{
	uint8_t rc;
...
uint8_t PnP_Module_4Input::AssignInterface(uint8_t index, PnP_InputInterface* pIfInstance)
{
...
PnP_InputInterface* PnP_Module_4Input::GetCurrentInterface()
{
	if (isInterfaceAssigned & 1<<currentEventIndex) {
